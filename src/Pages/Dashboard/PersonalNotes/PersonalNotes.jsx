        ...
        <div>
            ...
            <section className="container mx-auto">
                <div className="flex items-center gap-x-3 mb-12 text-2xl mx-4 my-4 font-bold">
                    ...
                    <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-stone-800 dark:text-blue-400"> {notes.length}</span>
                </div>

                <div className="flex flex-col mt-6">
                    {notes.length === 0 ? (
                        <div className="text-center py-6">
                            <h2 className="text-lg font-medium text-stone-800 dark:text-white">
                                No notes available
                            </h2>
                        </div>
                    ) : (
                            ...
                            <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                                <div className="overflow-hidden border border-stone-200 dark:border-stone-700 md:rounded-lg">
                                    <table className="min-w-full divide-y divide-stone-200 dark:divide-stone-700">
                                        ...
                                        {notes.map(note => (
                                            <tbody key={note._id} className="bg-white divide-y divide-stone-200 dark:divide-stone-700 dark:bg-stone-900">
                                                <tr>
                                                    <td className="px-4 py-4 text-sm font-medium text-stone-700 whitespace-nowrap">
                                                        <div className="inline-flex items-center gap-x-3">
                                                            <div className="flex items-center gap-x-2">
                                                                <div>
                                                                    <h2 className="font-medium text-stone-800 dark:text-white ">{note.title && note.title.length > 20
                                                    ...
                                                    <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                        <div className="flex items-center gap-x-6">
                                                            <Link to={`/dashboard/update-note/${note._id}`}>
                                                                <button className="text-stone-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-stone-300 hover:text-red-500 focus:outline-none">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
                                                                        <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                                                    </svg>
                                                                </button>
                                                            </Link>
                                                            <button onClick={() => handleDeleteNote(note)} className="text-stone-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-stone-300 hover:text-red-500 focus:outline-none">
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
